...
// ─── Auto-maintain listing-page card rows (preserve + auto-add) ───
//
// These listing sections (homepage Latest, news.html, comparisons.html,
// reviews.html) are hand-curated: each card carries a category, tag label,
// image, description and sometimes a rating that DON'T exist in the article
// files. So we never regenerate from scratch — we PRESERVE every existing
// card exactly (keyed by its URL) and only ADD a card for any article that
// doesn't have one yet, then sort the whole set by date (newest first).
//
// Injection happens only between these markers, so the rest of each page
// (filters, JS, hero, carousel) is never touched:
//   <!-- AUTO-CARDS:START --> ... <!-- AUTO-CARDS:END -->
// A page with no markers is left completely alone (safe no-op).

// Parse the existing cards inside the marker region, keyed by href.
// Returns { order: [href,...], cards: { href: {html, date} } }
function parseExistingCards(regionHtml) {
  const cards = {};
  const order = [];
  // Each card is an <article class="card" ...> ... </article> block.
  const re = /<article class="card"[^>]*>[\s\S]*?<\/article>/g;
  const blocks = regionHtml.match(re) || [];
  for (const block of blocks) {
    const href = (block.match(/<a href="([^"]+)"\s+class="card-media"/) || [])[1];
    if (!href) continue;
    const date = (block.match(/data-date="([^"]+)"/) || [])[1] || '';
    cards[href] = { html: block, date };
    order.push(href);
  }
  return { order, cards };
}

// Convert an article's urlPath to the href form a given page uses.
// Root pages (index/news/comparisons/reviews) reference reviews as
// "reviews/slug/" and news as "news-x.html" — i.e. no leading slash.
function pageHref(a) {
  return a.urlPath.replace(/^\//, '');
}

// Build a fresh card for an article that has no existing card yet.
// Uses conservative, correct defaults; a human can enrich later.
function buildNewCard(a, opts) {
  const href = pageHref(a);
  const date = a.datePublished || '';
  const label = a.kind === 'comparison' ? 'Comparison'
              : a.kind === 'news' ? 'News'
              : a.kind === 'guide' ? 'Guide'
              : 'Review';
  const eyebrow = a.kind === 'news' ? 'Just in'
                : a.kind === 'comparison' ? 'Comparison'
                : a.kind === 'guide' ? 'Guide' : 'Full review';
  const title = escapeHtml(a.title || deriveSitemapLabel(a));
  // Image: prefer an explicit hero if we can guess it, else a neutral fallback.
  // We DON'T invent image names — use the article's own og:image basename.
  const img = a.ogImage ? a.ogImage.split('/').pop() : 'images/logo.png';
  const imgPath = img.startsWith('images/') ? img : 'images/' + img;
  const cat = opts.withCategory ? ` data-category="${a.autoCategory || 'all'}"` : '';
  const gauge = opts.withGauge
    ? '\n              <div class="mini-gauge"><span class="dot" style="background:var(--ink-soft)"></span> ' + label + '</div>'
    : '';
  return `        <article class="card"${cat} data-date="${date}">
...
// Regenerate the marker region for one page.
// filterFn decides which articles belong on this page.
// opts.withCategory / opts.withGauge match the page's card style.
function updateListingPage(relPath, articles, filterFn, opts) {
  let html;
  try { html = readFile(relPath); } catch { return false; }

  const START = '<!-- AUTO-CARDS:START -->';
  const END = '<!-- AUTO-CARDS:END -->';
  const si = html.indexOf(START);
  const ei = html.indexOf(END);
  if (si === -1 || ei === -1 || ei < si) return false; // no markers → leave page alone

  const before = html.slice(0, si + START.length);
  const region = html.slice(si + START.length, ei);
  const after = html.slice(ei);

  const existing = parseExistingCards(region);

  // Which articles belong here?
  const relevant = articles.filter(filterFn);

  // Build final ordered list: keep existing card HTML where present,
  // create new cards for articles that lack one.
  const merged = [];
  for (const a of relevant) {
    const href = pageHref(a);
    if (existing.cards[href]) {
      const date = existing.cards[href].date || a.datePublished || '';
      merged.push({ href, date, html: ensureDataDate(existing.cards[href].html, date) });
    } else {
      merged.push({ href, date: a.datePublished || '', html: buildNewCard(a, opts) });
    }
  }

  // Also keep any existing cards whose URL didn't match an article
  // (e.g. manually-added external/other cards) so we never delete curation.
  for (const href of existing.order) {
    if (!merged.find(m => m.href === href)) {
      const date = existing.cards[href].date || '';
      merged.push({ href, date, html: ensureDataDate(existing.cards[href].html, date) });
    }
  }

  // Sort newest first by date; undated cards sink to the bottom in original order.
  merged.sort((a, b) => (b.date || '').localeCompare(a.date || ''));

  // Preserve each card's original whitespace exactly (no re-indenting —
  // cosmetic indent changes risk compounding and don't affect rendering).
  const rebuilt = '\n        ' + merged.map(m => m.html.trim()).join('\n\n        ') + '\n      ';
  const out = before + rebuilt + after;
  if (out !== html) writeFile(relPath, out);
  return true;
}

// Inject a data-date attribute onto a card's <article> tag if it lacks one,
// so ordering becomes stable and self-describing on future runs.
function ensureDataDate(cardHtml, date) {
  if (!date) return cardHtml;
  if (/<article class="card"[^>]*data-date=/.test(cardHtml)) return cardHtml;
  return cardHtml.replace(/<article class="card"([^>]*)>/,
    `<article class="card"$1 data-date="${date}">`);
}

function updateAllListingPages(articles) {
  const results = [];
  // Homepage "Latest" is a CURATED subset, not every article. So here we only
  // preserve existing cards and keep them sorted — we do NOT auto-add every
  // article (that would flood the homepage). New featured cards are added by
  // hand; this just keeps order correct and stamps data-date. Passing a filter
  // that matches nothing means "add nothing new, preserve+sort what's there".
  if (updateListingPage('index.html', articles, () => false, { withCategory: false, withGauge: false }))
    results.push('index.html');
  // News page: news only — auto-adds any missing news article
  if (updateListingPage('news.html', articles, a => a.kind === 'news', { withCategory: false, withGauge: false }))
    results.push('news.html');
  // Comparisons page: comparisons only (cards use data-category + gauge)
  if (updateListingPage('comparisons.html', articles, a => a.kind === 'comparison', { withCategory: true, withGauge: true }))
    results.push('comparisons.html');
  // Reviews page: reviews + guides (cards use data-category + gauge)
  if (updateListingPage('reviews.html', articles, a => a.kind === 'review' || a.kind === 'guide', { withCategory: true, withGauge: true }))
    results.push('reviews.html');
  return results;
}
...
function main() {
  ...
  const listingPages = updateAllListingPages(articles);
  if (listingPages.length) {
    console.log(`  listing cards synced — ${listingPages.join(', ')}`);
  }
